...
app.post('/api/segment', async (req, res) => {
  try {
    ...
    const result = await runPythonScript('segment.py');
    
    console.log('Clustering completed successfully');
    console.log(`Number of clusters: ${result.n_clusters}`);
 ...
 * Predict cluster segment for an employee
 */
app.post('/api/segment/predict', async (req, res) => {
  try {
    const employeeData = req.body;
    ...
    const modelPath = path.join(__dirname, 'models', 'clustering_model.pkl');
    if (!fs.existsSync(modelPath)) {
      return res.status(400).json({
        success: false,
        error: 'Clustering model not found. Please train the model first using /api/segment endpoint'
      });
    }

    console.log('Predicting employee segment...');
    
    const result = await runPythonScript('segment_predict.py', [JSON.stringify(employeeData)]);
    ...
    console.error('Segmentation prediction error:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to predict segment'
    });
  }
});
...
app.get('/api/clusters', async (req, res) => {
  try {
    const profilesPath = path.join(__dirname, 'models', 'cluster_profiles.json');
    
    if (!fs.existsSync(profilesPath)) {
      return res.status(400).json({
        success: false,
        error: 'Cluster profiles not found. Please train the clustering model first'
      });
    }
...
/**
 * POST /api/recommend
 * Get personalized recommendations for an employee
 */
app.post('/api/recommend', async (req, res) => {
  try {
    const employeeData = req.body;
    ...
    const attritionModelPath = path.join(__dirname, 'models', 'best_model.pkl');
    const clusteringModelPath = path.join(__dirname, 'models', 'clustering_model.pkl');
    
    if (!fs.existsSync(attritionModelPath)) {
      return res.status(400).json({
        success: false,
        error: 'Attrition model not found. Please train the model first'
      });
    }
    
    if (!fs.existsSync(clusteringModelPath)) {
      return res.status(400).json({
        success: false,
        error: 'Clustering model not found. Please train the model first'
      });
    }

    console.log('Generating recommendations...');
    
    const result = await runPythonScript('recommend.py', [JSON.stringify(employeeData)]);
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      segmentPredict: 'POST /api/segment/predict',
